...
export const InterceptLogsPage = () => {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const { data } = await api.get<InterceptLogItem[]>("/api/admin/client-intercept-logs?limit=100");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-slate-900 dark:text-white">拦截日志</h2>
        <button
          type="button"
          onClick={load}
          ...
          <table className="w-full text-xs">
            <thead className="bg-slate-100 dark:bg-slate-800/80 text-slate-700 dark:text-slate-300">
              <tr>
                ...
                <th className="px-3 py-2 text-left">客户端</th>
                <th className="px-3 py-2 text-left">类型</th>
                <th className="px-3 py-2 text-left">上游</th>
